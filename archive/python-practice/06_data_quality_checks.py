...
def plot_results(monthly: pd.DataFrame) -> None:
    ...
    plt.ylabel("Выручка")
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "quality_revenue.png", dpi=160)
    plt.close()
    ...
    plt.ylabel("ROMI")
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "quality_romi.png", dpi=160)
    plt.close()
...
def save_report(monthly: pd.DataFrame) -> None:
    ...
    report = f"""# Отчёт по качеству данных
...
"""
    (REPORTS_DIR / "quality_conclusion.md").write_text(report, encoding="utf-8")
