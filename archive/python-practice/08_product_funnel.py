...
def plot_results(monthly: pd.DataFrame) -> None:
    ...
    plt.ylabel("Выручка")
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "funnel_revenue.png", dpi=160)
    plt.close()
    ...
    plt.ylabel("ROMI")
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "funnel_romi.png", dpi=160)
    plt.close()
...
"""
    (REPORTS_DIR / "funnel_conclusion.md").write_text(report, encoding="utf-8")
