...
def plot_results(monthly: pd.DataFrame) -> None:
    ...
    plt.ylabel("Выручка")
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "churn_revenue.png", dpi=160)
    plt.close()
    ...
    plt.ylabel("ROMI")
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "churn_romi.png", dpi=160)
    plt.close()
...
def save_report(monthly: pd.DataFrame) -> None:
    ...
    report = f"""# Признаки для анализа оттока
...
"""
    (REPORTS_DIR / "churn_conclusion.md").write_text(report, encoding="utf-8")
